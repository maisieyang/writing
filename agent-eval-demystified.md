...
3. **Dogfood**：把完整产品放回真实使用，感受它到底是否好用，并发现已有测试和 Eval 尚未描述的问题。
4. **公共 Benchmark**：当系统已经基本成形，再把它放到一组拥有外部答案、其他系统也在运行的公共任务上，获得一个可以对外比较的坐标。

这四项并不是频率相同、依次升级的四层测试。前三项构成一次开发应该走完的日常闭环：TDD 锁住确定性的机制行为，Eval 管理模型决策的概率性，Dogfood 验证完整体验并暴露未知问题。Benchmark 通常不需要在每次开发时运行；它更像阶段性的外部考试。当我自己觉得“已经差不多了”，才需要拿一把大家共同使用的尺子，看看这个系统放到外部究竟处在什么位置。

Eval 和 TDD 不一样的地方是，被测行为开始由模型参与，结果存在概率波动，因此需要另一套 Dataset、Oracle 和稳定性证据。

Dogfood 在这个闭环里尤其重要。Eval 只能覆盖我已经知道应该关注什么；Dogfood 才会暴露我不知道自己不知道什么。
...
## Dogfood 推动开发，Benchmark 提供外部坐标

Dogfood 是我会持续进行的日常工作。每完成一段能力，我都需要把它放回完整产品里使用，观察各个局部契约组合起来以后是否仍然成立。公共 Benchmark 则不是每次开发都要运行的回归环节。它成本更高，也更适合在系统已经基本成形、我自己觉得“差不多了”的时候运行。

这时我需要回答的已经不只是“它是否符合我为自己写下的要求”，而是“把它放到外部，它究竟表现得怎么样”。公共 Benchmark 的价值，就在于提供一组其他模型和 Agent 系统也会运行的任务，以及一套共享的评判方式。大家在同一把尺子上得到分数，外部才有一个相对可比的评价标准。“谁更厉害”不再只靠每个项目讲述自己的成功案例，而有了一组共同结果可以讨论。

但公共的尺子并不等于绝对能力。Benchmark 测到的并不是一个裸模型，而是模型在特定 Agent 系统和运行条件下的整体表现。因此，分数只有和这些条件放在一起才有意义。在这个前提下，公共 Benchmark 仍然提供了内部 Eval 无法替代的外部坐标：它让我知道自己的系统放进共同题库后处在什么位置，而不只是通过自己出的题证明自己不错。

OpenHarness 曾通过产品实际使用的运行时跑完 SWE-bench Lite 的全部 300 个任务。在一个固定基线上，qwen3.7-max 最终解决了 170 个，得到 56.7% 的 resolved rate。

这个数字当然重要，但这次 Benchmark 给我的最大收获并不只有 56.7%。

我后来分析运行记录，发现成功任务和失败任务的 turn 数分布非常接近。一个 Agent 可以工作很多轮、调用很多工具、生成一份看起来合理的 Patch，最后仍然是错的。

工作时间不是完成证据，模型的自信也不是完成证据。

这个发现后来成为 OpenHarness `/goal` 设计的重要依据：负责执行任务的模型不能独自宣布自己已经完成。完成判断必须交给独立的外部 oracle，根据事先声明的条件和真实工具证据作出决定。

Benchmark 很擅长告诉我“在公共任务上最终有没有做成”，却不一定告诉我为什么。Capability Eval 可以隔离某一个决策面，却只能测量已经被定义的问题。Dogfood 无法提供整齐的分数，却会暴露交互中的迟钝、误解和不诚实感。TDD、Eval 和 Dogfood 构成开发闭环；阶段性的 Benchmark 再把这个闭环产生的系统放到外部坐标中检验。它们不是四个并列的测试工具，而是承担着不同角色的证据来源。
